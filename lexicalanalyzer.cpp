...
int main(){
	...
	/*
		Ifstream object is used for reading the file ofstream object
		is used to write the file in your code.
	*/
    ifstream fin("lexicalinput.txt");
    int mark[1000]={0};
    int i,j=0,kc=0,ic=0,lc=0,mc=0,nc=0,oc=0,aaa=0;
	/*
		C++ Vector. A vector is a sequence container class that implements dynamic array,
	...
	*/
	...
	/*
		 check if the stream is has raised any EOF (End Of File) error.
	*/
    while(!fin.eof()){
           ch = fin.get();
          for(i = 0; i < 12; ++i){
